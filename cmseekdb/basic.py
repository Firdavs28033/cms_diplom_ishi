#!/usr/bin/python3
# -*- coding: utf-8 -*-
# This is a part of DIPLOM ISHI CMS, check the LICENSE file for more information
# Copyright (c) 2018 - 2020 Firdavs28033
# Thought this file was getting quite bloated so refactored it
...
skip_scanned = False # When set to true DIPLOM ISHI CMS witll ignore target whose CMS had been previously detected!
...
if access_directory == "" or not os.path.exists(access_directory):
    ...
    if os.access(cmseek_dir, os.W_OK):
        # use the parent DIPLOM ISHI CMS directory if it is writeable
        access_directory = cmseek_dir
    else:
        if cmseek_dir == os.getcwd():
            # current directory and DIPLOM ISHI CMS directory are same and write access not available. show error if --batch is not used
            if not batch_mode:
...
def banner(txt):
    # The sexy banner for DIPLOM ISHI CMS!!!
    global cmseek_version
    print(bold + fgreen + """ 

  _____ _______      __           _             _                _                 _   ___   ___ ___  _____ 
 |_   _|_   _\ \    / /     /\   | |           | |              (_)               (_) |__ \ / _ \__ \| ____|
   | |   | |  \ \  / /     /  \  | | ____ _  __| | ___ _ __ ___  _ _   _  __ _ ___ _     ) | | | | ) | |__  
   | |   | |   \ \/ /     / /\ \ | |/ / _` |/ _` |/ _ \ '_ ` _ \| | | | |/ _` / __| |   / /| | | |/ /|___ \ 
  _| |_ _| |_   \  /     / ____ \|   < (_| | (_| |  __/ | | | | | | |_| | (_| \__ \ |  / /_| |_| / /_ ___) |
 |_____|_____|   \/     /_/    \_\_|\_\__,_|\__,_|\___|_| |_| |_|_|\__, |\__,_|___/_| |____|\___/____|____/ 
                                                                    __/ |                                   
                                                                   |___/                                    
...
""".format(orange, lblue, yellow, cmseek_version, red, white))
    if txt != "":
        ...
        print(cln + bold + lbluebg + black + " Author: " + cln + bold + " Firdavs Abdurasulov" + blackbg + white + "\n GitHub: " + cln + bold + " https://github.com/Firdavs28033 \n" + cln + '\n')
    print(cln)
    return
...
def help():
    ...
    print(
    """
DIPLOM ISHI CMS Version {0}
Github: {4}
Coded By:{1}{3} @Firdavs28033 {2}

USAGE:
       python3 diplom_ishi_cms.py (for guided scanning) OR
       python3 diplom_ishi_cms.py [OPTIONS] <Target Specification>
...
VERSION & UPDATING:
      --update                     Update DIPLOM ISHI CMS (Requires git)
      --version                    Show DIPLOM ISHI CMS version and exit
...
EXAMPLE USAGE:
      python3 diplom_ishi_cms.py -u example.com                           # Scan example.com
      python3 diplom_ishi_cms.py -l /home/user/target.txt                 # Scan the sites specified in target.txt (comma separated)
      python3 diplom_ishi_cms.py -u example.com --user-agent Mozilla 5.0  # Scan example.com using custom user-Agent Mozilla is 5.0 used here
      python3 diplom_ishi_cms.py -u example.com --random-agent            # Scan example.com using a random user-Agent
      python3 diplom_ishi_cms.py -v -u example.com                        # enabling verbose output while scanning example.com
...
def bye():
    ...
    print('\n' + bold + red + ' DIPLOM ISHI CMS says ~ ' + this_time + cln)
    quit()
...
def update_brute_cache():
    ...
    if not os.path.isdir(brute_dir):
        try:
            error('DIPLOM ISHI CMS could not find the bruteforce directory, Creating Brute directory')
            os.makedirs(brute_dir)
            info('Bruteforce directory created, add some modules from: https://github.com/Firdavs28033/diplom_ishi_cms')
            bye()
    ...
    if not modules == [] and modulen != []:
        ...
        print('\n')
        result('Cache Updated! Enjoy DIPLOM ISHI CMS with new modules ;)','')
    else:
...
def update():
    ...
    get_version = getsource('https://raw.githubusercontent.com/Firdavs28033/DIPLOM-ISHI-CMS/master/current_version',randomua('generate'))
    if get_version[0] != '1':
        ...
        info("DIPLOM ISHI CMS Version: " + cmseek_version)
        ...
        if my_version > serv_version:
            ...
            print('\n')
            result("DIPLOM ISHI CMS is up to date, Thanks for checking update tho.. It's a good practise",'')
        else:
            print('\n')
            ...
            if update_me.lower() == 'y':
                ...
                if succes == True:
                    result("DIPLOM ISHI CMS Updated To Latest Version! Enjoy", "")
                else:
...
def savebrute(url,adminurl,username,password):
    ...
    if url != "" and adminurl != "" and username != "" and password != "":
        ...
        brute_result = "### DIPLOM ISHI CMS Bruteforce Result\n\n\nSite: " + url + "\n\nLogin URL: " + adminurl + "\n\nUsername: " + username + "\n\nPassword: " + password
